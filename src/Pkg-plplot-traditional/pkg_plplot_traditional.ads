...
package pkg_plplot_traditional is

    -- Used with plsdidev.
    PL_NOTSET : Long_Float renames PKG_PLPLOT_THIN.PL_NOTSET;

    -- "Rename" some useful types from the thin binding.
    subtype Boolean_Array_1D is PKG_PLPLOT_THIN.Boolean_Array_1D;
    subtype Integer_Array_1D is PKG_PLPLOT_THIN.Integer_Array_1D;

    -- "Rename" some types mainly used for contour plots and the like so that
    -- user programs can see them without with-ing PKG_PLPLOT_THIN. It might be
    -- good to remove "use PKG_PLPLOT_THIN" for clarity since it is used in only
    -- a few places.
    subtype PL_Pointer                 is Pkg_Plplot_Thin.PL_Pointer;
    ...
    subtype PLpointer                  is Pkg_Plplot_Thin.PLpointer; -- DEPRECATED
    subtype Unsigned_Int               is Interfaces.C.unsigned;
    subtype Transformation_Data_Type   is Pkg_Plplot_Thin.Transformation_Data_Type;
    subtype Transformation_Data_Type_2 is Pkg_Plplot_Thin.Transformation_Data_Type_2;
    subtype Graphics_Input_Record_Type is Pkg_Plplot_Thin.PLGraphicsIn;
    -- subtype Data_Ops_2D_Type           is Pkg_Plplot_Thin.PLf2ops_t; -- Manipulate 2D data

    -- "Rename" the unicode type for characters.
    subtype Unicode is PKG_PLPLOT_THIN.PLUNICODE;

    -- Rename Pkg_Plplot_Thin.plfill so that a pointer to it can be passed to
    -- procedures such as plshade and plshades and called with C
    -- conventions. Note that Pkg_Plplot_Thin.plfill is already in C so that it does
    ...
    pragma Warnings(Off);
    procedure plfill(length : Integer; x, y : Real_Vector) renames PKG_PLPLOT_THIN.plfill;
    pragma Warnings(On);

    -- Make Mask_Function_Pointer_Type available to the user so that s/he doesn't
    -- have to "with" PKG_PLPLOT_THIN. Note that it is also used herein.
    subtype Mask_Function_Pointer_Type is PKG_PLPLOT_THIN.Mask_Function_Pointer_Type;

    -- "Rename" callback for map functions plmap and plmeridians.
    subtype Map_Form_Function_Pointer_Type is Pkg_Plplot_Thin.Map_Form_Function_Pointer_Type;

    -- "Rename" the necessarily constrained array for mapping longitudes and latitudes.
    subtype Map_Form_Constrained_Array is PKG_PLPLOT_THIN.Map_Form_Constrained_Array;
    ...
    Max_Label_String_Length : Integer renames PKG_PLPLOT_THIN.Max_Label_String_Length;
    subtype Label_String_Type is PKG_PLPLOT_THIN.Label_String_Type;

    -- "Rename" callback for custom label functions.
    subtype Custom_Label_Procedure_Pointer_Type is
        PKG_PLPLOT_THIN.Custom_Label_Procedure_Pointer_Type;

    -- "Rename" callback for custom coordinate transform procedure.
    subtype Coordinate_Transform_Procedure_Pointer_Type is
        PKG_PLPLOT_THIN.Coordinate_Transform_Procedure_Pointer_Type;
...
--------------------------------------------------------------------------------
    ...
    -- (duplicated from pkg_plplot_thin.ads)
    ...
    -- Flags for pllegend (duplicated from pkg_plplot_thin.ads)
    ...
    -- Flags for plcolorbar (duplicated from pkg_plplot_thin.ads)
...
--------------------------------------------------------------------------------
-- Constants copied from PKG_PLPLOT_THIN.ads so that it doesn't have to be seen.  --
...
--  EXCEPT FOR THE FOLLOWING.
    ...
    -- of pltr? from pkg_plplot_thin.adb into plplot_traditional.adb and
